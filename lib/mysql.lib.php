...
class DB
{
            ...
            if ( false===$res) {
                ...
                if ($errno ==2006 || $error == 2013) {
                    try {
                        $this->db = $this->connect($throw);
                        if ( false === $this->db ) {
                            _LOG("[MYSQLD] MySQL reconnect false. $sql");
                        } else {
                            _LOG("[MYSQLD] MySQL reconnect done . $sql");
                        }
                    } catch (Exception $e) {
                        _LOG("[MYSQLD] MySQL reconnect failed. $sql");
                        if ( $throw ) throw new Exception("MYSQL reconnect failed. $sql");
                        return false;
                    }
                } else {
                    _LOG("[MYSQLD][db-".__LINE__."] MySQL query failed ".$this->errno." , ".$this->error." , ".$sql);
    ...
    {
        if ( !$query ) return _LOG("[MYSQLD][db-".__LINE__."] MySQL query failed 0, , ".$query);
        if (!$this->db) {
            try {
                $this->db = $this->connect($throw);
            } catch (Exception $e) {
        ...
        for ($i = 0; $i < 2; $i++)
        {
            ...
            if ( false === $res ) {
                ...
                if ($errno ==2006 || $error == 2013) {
                    ...
                    _LOG("[MYSQLD][db-".__LINE__."] MySQL query failed ".$this->errno." , ".$this->error." , ".$query);
                    if ( $throw ) throw new Exception("MYSQL query failed.");
                    return false;
                }
            } else {
    ...
    function insert($table, $array = array())
    {
        ...
        $values = [];
        foreach(array_values($array) as $vl){
            $values[] = addslashes($vl);
        }

        $sql .= "(`" . implode('`,`', $fields) . "`)VALUES ('" . implode("','", $values) . "')";
        _LOG("MySQL insert" .json_encode($array).$sql);

        return $this->runSql($sql);
    }
    function batchinsert($table, $array = array())
    {
        $sql = " INSERT INTO {$table} ";
    ...
    public function update($table, $aFields, $where = '',$prepare_array=array())
    {
        ...
        if($this->runSql($sql))return true;
        return false;
    }
